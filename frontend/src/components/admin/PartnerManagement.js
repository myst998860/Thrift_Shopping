...
const PartnerManagement = () => {
  ...
  const [searchTerm, setSearchTerm] = useState('');
  const [actionMenu, setActionMenu] = useState(null);
  const [previewSrc, setPreviewSrc] = useState(null); // image modal
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const navigate = useNavigate();
  ...
  const handleSuspend = async (id) => {
    try {
      await partnerService.updatePartnerStatus(id, 'Inactive');
      setPartnersData(partnersData.map(p => p.user_id === id ? { ...p, status: 'Inactive' } : p));
      setActionMenu(null);
    } catch (error) {
      console.error("Failed to suspend partner:", error);
    }
  ...
  const handleEditPartner = (partner) => {
    if (!partner?.user_id) return console.error("Invalid partner ID:", partner?.user_id);
    navigate(`/admin/partners/edit/${partner.user_id}`);
  };
    ...
    <div className="partner-management">
      ...
      <div className="user-list-container">
        ...
        {activeTab === 'Pending' ? (
          ...
          /* TABLE VIEW FOR ALL OR VERIFIED */
          <div className="partner-table-container">
            <table>
              <thead>
                <tr>
      ...
      {previewSrc && (
        <div className="image-modal" onClick={() => setPreviewSrc(null)}>
          <div className="image-modal__content" onClick={(e) => e.stopPropagation()}>
            <button className="image-modal__close" onClick={() => setPreviewSrc(null)}>×</button>
            <img src={previewSrc} alt="Document preview" />
          </div>
        </div>
      )}
    </div>
